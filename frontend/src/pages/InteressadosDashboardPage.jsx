        ...
        const years = Array.from(
          new Set(
            (data.monthlyPerformance || []).map(
              (item) => String(item.month).split("-")[0]
            )
          )
        ).sort();
        if (years.length > 0) {
  ...
  const DashboardContent = () => {
    ...
    const monthlyPerformanceData = {
      ...
      datasets: [
        {
          label: "Ganhos",
          data: filteredMonthly.map((item) => parseInt(item.ganhos, 10)),
        ...
        {
          label: "Perdidos",
          data: filteredMonthly.map((item) => parseInt(item.perdidos, 10)),
      ...
      >
        ...
        <div style={{ ...chartContainerStyle }}>
          ...
          >
            ...
            <div>
              <label htmlFor="yearSelect" style={{ marginRight: 8 }}>
                Ano:
              </label>
              <select
                id="yearSelect"
